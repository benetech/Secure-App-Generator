...
import org.martus.android.vitalVoices.tasks.CreateMartusCryptoKeyPairTask;
...
public class CreatePassphraseActivity extends AbstractLoginActivity implements ICacheWordSubscriber, TextWatcher {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        passphraseEditField = (FloatLabel) findViewById(R.id.passphrase_edit_field);
        passphraseConfirmationEditField = (FloatLabel) findViewById(R.id.passphrase_confirm_edit_field);
    ...
    private void safelyClearOdkDbFilesFromPreviousAccount() {
        try {
            ...
            Toast.makeText(getApplicationContext(), getString(R.string.error_cleaning_up_odk_folders), Toast.LENGTH_LONG).show();
        }
    }
    ...
    private void clearOdkDbFilesFromPreviousAccount() throws Exception {
        ...
        for (String dirPath : odkPathsToDelete) {
            ...
            if (odkDirToDelete.exists())
                throw new Exception("Could not clear odk folders from previous account");
            else
                Log.i(TAG, "ODK folders deleted as part of cleanup: " + odkDirToDelete.getAbsolutePath());
        }
    }
    ...
    private int getPossibleValidErrorMessage() {
        if (!doPassPhrasesMatch())
            return R.string.settings_pwd_not_equal;

        if (!isValidPassPhraseLength())
            return R.string.error_incorrect_passphrase;

        if (!containsOnlyValidChars())
            return R.string.error_message_invalid_chars;
    ...
    public void afterTextChanged(Editable s)
    {
        ...
        if (password.length < 8) {
            passphraseEditField.getEditText().setError(getString(R.string.invalid_password));
            return;
        }
        if (Arrays.equals(password, confirmPassword)) {
            createAccountButton.setEnabled(true);
        } else {
            passphraseConfirmationEditField.getEditText().setError(getString(R.string.settings_pwd_not_equal));
        }
    }
    ...
    protected void postMountStorageExecute() {
        ...
        try {
            ...
            Log.e("uploadForm", "Failed to create account", e);
        }
        finally {
            Wiper.wipe(passphrase);
        }
    ...
    private CreateMartusCryptoKeyPairTask.CreateMartusCryptoKeyPairCallback mCreateMartusCryptoKeyPairCallback = new CreateMartusCryptoKeyPairTask.CreateMartusCryptoKeyPairCallback() {
        ...
        public void onCreateKeyPairError() {
            ...
            throw new RuntimeException("Counld not create martus crypto key pair");
        }
